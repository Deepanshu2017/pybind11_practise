...
PYBIND11_MODULE(foo, m) {
  py::class_< fWrapper<N_> >(m, "fw")
    .def(py::init<std::array<double, N_>const&, unsigned long long>())
    .def(py::init<py::array_t<double, py::array::c_style>&, unsigned long long>())
    .def("__call__", [](fWrapper<N_> &f, double x) {
	f(&x);
      }, py::is_operator())
    .def("__call__", [](fWrapper<N_> &f, py::array_t<double, py::array::c_style>& arr, double x) {
	f(arr, &x);
      }, py::is_operator())
    .def("__call__", [](fWrapper<N_> &f, std::array<double, N_>& arr, double x) {
	f(arr, &x);
      }, py::is_operator())
    ;
}
